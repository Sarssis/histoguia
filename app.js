...
const KEY='histoguia-v12-state', THEME='histoguia-v12-theme';
...
function appleLink(e){return e.appleUrl && e.appleUrl.trim() ? e.appleUrl : 'https://podcasts.apple.com/search?term='+encodeURIComponent('HistoCast '+e.hc.replace('HC','')+' '+e.title)}
function appleLabel(e){return e.appleUrl && e.appleUrl.trim() ? 'Abrir episodio' : 'Buscar en Apple Podcasts'}
...
function hero(){
 ...
 return `<section class="card hero"><div><p class="overline">Continuar</p><h2>${n.hc} · ${n.title}</h2><p class="meta">${n.period} · ${n.era}</p><div class="actions"><a class="primary" href="${appleLink(n)}" target="_blank">${appleLabel(n)}</a><button class="secondary" onclick="markNext()">Marcar escuchado</button></div></div><div class="ring"><svg viewBox="0 0 120 120"><circle class="track" cx="60" cy="60" r="52"/><circle class="meter" style="stroke-dashoffset:${327-(c.pct/100)*327}" cx="60" cy="60" r="52"/></svg><b>${c.pct}%</b></div></section>
...
function episodeCard(e){
 const s=status(e);
 return `<article class="episode ${s}"><button class="check" onclick="cycleById(${e.id})">${s==='done'?'✓':s==='listening'?'▶':''}</button><div><div class="topline"><span class="code">${String(e.id).padStart(2,'0')} · ${e.hc}</span><button class="star" onclick="favById(${e.id})">${favorite(e)?'★':'☆'}</button></div><h3>${e.title}</h3><p class="meta">${e.period} · ${e.era}</p><div class="tags"><span class="tag">${pri(e.priority)}</span><span class="tag">${e.type}</span></div><div class="row"><a class="mini" href="${appleLink(e)}" target="_blank">${appleLabel(e)}</a><button class="mini" onclick="cycleById(${e.id})">${s==='done'?'Escuchado':s==='listening'?'Escuchando':'Pendiente'}</button></div></div></article>`
}
...
function library(){const podcasts=['HistoCast','Roma Aeterna','SER Historia','Hardcore History','La Biblioteca Perdida'];screen.innerHTML=`<div class="section-title"><div><h2>Biblioteca</h2><p>Base preparada para añadir más fuentes.</p></div></div><section class="list">${podcasts.map((p,i)=>`<article class="period-card"><b>${p}</b><span>${i===0?EPISODES.length+' episodios cargados':'Pendiente de integrar'}</span></article>`).join('')}</section>`}
